...
function getCpuUsage(): number {
  ...
  if (!lastCpuTicks) {
    ...
    return 0; // Need a second sample for a real percentage
  }
  const idleDiff = idle - lastCpuTicks.idle;
  const totalDiff = total - lastCpuTicks.total;
  lastCpuTicks = { idle, total };
  return totalDiff === 0 ? 0 : 100 - Math.floor((idleDiff / totalDiff) * 100);
}

async function computeVitals() {
  const [claude, openclaw, hermes, gemini, antigravity] = await Promise.all([
    run("claude", ["--version"], { timeoutMs: 6000 }),
    run("openclaw", ["health"], { timeoutMs: 6000 }),
    run("hermes", ["status"], { timeoutMs: 8000 }),
    run("gemini", ["--version"], { timeoutMs: 6000 }),
    run("antigravity", ["--version"], { timeoutMs: 6000 }),
  ]);
  return { claude, openclaw, hermes, gemini, antigravity };
}
...
export async function GET() {
  ...
  const { claude, openclaw, hermes, gemini, antigravity } = (await inflight) as Awaited<ReturnType<typeof computeVitals>>;
  ...
  const body = {
    ...
    gemini: {
      ok: gemini.ok,
      version: gemini.stdout.trim() || gemini.stderr.trim(),
