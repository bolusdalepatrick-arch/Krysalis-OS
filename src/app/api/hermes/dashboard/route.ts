...
// Lifecycle for the Hermes web dashboard (FastAPI on :9119). Krysalis OS embeds it
...
// leaving Krysalis OS. GET = status; POST = ensure it's running (start if down).
...
export async function POST() {
  ...
  try {
    // --tui enables the embedded Chat pane; --no-open keeps it headless (we frame it).
    const child = spawn(bin, ["dashboard", "--tui", "--no-open", "--port", "9119"], {
      detached: true,
      stdio: "ignore",
      env: { ...process.env },
    });
