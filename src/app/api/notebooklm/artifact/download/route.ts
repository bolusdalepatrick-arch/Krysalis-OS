...
const ASSETS_DIR = path.join(os.homedir(), "Documents", "Obsidian Vault", "Krysalis OS", "Notebooks", "_assets");
...
function extFor(artifact_type: string): string {
  switch (artifact_type) {
    ...
    case "mind_map":    return ".html";
    case "flashcards":
    case "quiz":
    case "data_table":  return ".json";
    default:            return ".bin";
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const result = await callTool<{ filePath?: string; file_path?: string; path?: string; error?: string; status?: string }>("download_artifact", args);
    if (result?.error || result?.status === "error") {
      return NextResponse.json({ error: result.error || "download failed", details: result }, { status: 422 });
    }
    const savedPath = result?.filePath ?? result?.file_path ?? result?.path ?? (existsSync(output_path) ? output_path : null);
