...
//
// Chat threads are stored per-agent under `krysalis-os-chat-v2:<agent>` as a
...
const key = (a: string) => `krysalis-os-chat-v2:${a}`;
...
const DEMO: Record<string, Msg[]> = {
  ...
  hermes: thread("hermes", [
    ["I just built 5 interactive, zero-dependency canvas demos in my Hermes workspace for an Krysalis OS showcase: (1) Cosmic Galaxy - a mouse-reactive 2,600-particle spiral galaxy with click-to-supernova, (2) Flow Field Art - a generative vector-field painting that reseeds on click, (3) Neon Spectrum - a 96-band radial audio visualizer with a beat-synced core, (4) Global Mesh - a rotating 3D wireframe globe with travelling data arcs, (5) Dopamine Fireworks - click-anywhere fireworks with trails and gravity. Give me a punchy one-line pitch for each, then tell me which one to open first for maximum wow.",
  ...
  gemini: thread("gemini", [
    ["Summarize this 80-page PDF into 5 bullets",
     "Here are the 5 key takeaways: ① revenue up 24% YoY driven by enterprise, ② churn improved to 1.9%, ③ EMEA is the fastest-growing region, ④ R&D spend front-loaded for the AI roadmap, ⑤ guidance raised for next quarter. Want the supporting figures per point?"],
    ["Compare these two screenshots and tell me what changed",
     "Three differences: the CTA moved above the fold, the nav lost the “Pricing” link, and the hero switched from a static image to a gradient. The second design has stronger visual hierarchy but weaker discoverability for pricing."],
  ]),
...
export default function DemoHistory() {
  ...
  const labels: Record<string, string> = {
    ...
    gemini: "Gemini", antigravity: "Antigravity", codex: "Codex",
  };
