// Video project + render job state — for the /video tab in Krysalis OS.
//
// HyperFrames projects scaffold under ~/.krysalis-os/video-projects/<slug>/
...
export const VIDEO_ROOT = path.join(HOME, ".krysalis-os", "video-projects");
export const RENDER_JOBS_FILE = path.join(HOME, ".krysalis-os", "video-render-jobs.json");
export const RENDER_LOGS_DIR = path.join(HOME, ".krysalis-os", "video-render-logs");
...
export async function listProjects(): Promise<HFProject[]> {
  ...
  for (const e of entries) {
    ...
    const meta = path.join(cwd, "krysalisos.meta.json");
    if (existsSync(meta)) {
...
export async function createProject(prompt: string, customSlug?: string): Promise<{ slug: string; cwd: string }> {
  ...
  await writeFile(path.join(cwd, "krysalisos.meta.json"), JSON.stringify({
    prompt, createdAt: Date.now(), source: "krysalisos/video",
  }, null, 2));
  return { slug, cwd };
}
