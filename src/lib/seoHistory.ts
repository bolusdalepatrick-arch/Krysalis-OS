...
// Persisted to ~/.krysalis-os/seo-history.json. Append-only with a rolling cap.
...
const HISTORY_FILE = path.join(os.homedir(), ".krysalis-os", "seo-history.json");
...
export interface DeployEvent {
  ...
  vercelUrl?: string;       // unique deploy preview URL parsed from vercel CLI output
  durationMs?: number;
  errorTail?: string;
}
...
export async function finishDeploy(deployId: string, fields: { status: "ok" | "failed"; vercelUrl?: string; errorTail?: string }): Promise<void> {
  const data = await load();
  ...
  if (fields.vercelUrl) d.vercelUrl = fields.vercelUrl;
