...
export const AGENTIC_DIR = VAULT_ROOT ? path.join(VAULT_ROOT, "Krysalis OS") : "";
...
// ─── MEMORIES ───────────────────────────────────────────────────────
// One file per day under Krysalis OS/Memories/YYYY-MM-DD.md
export async function appendMemory(entry: {
  ...
  const header = `---\ntags: [memory, krysalis-os, ${day}]\ndate: ${day}\n---\n\n# 🧠 Krysalis OS Memory — ${day}\n\n`;
  const block = [
...
export async function listMemoryDays(limit = 14): Promise<{ date: string; path: string }[]> {
  try {
    ...
    const days = items
      ...
      .reverse()
      .slice(0, limit);
    return days.map((d) => ({ date: d, path: `Krysalis OS/Memories/${d}.md` }));
  } catch { return []; }
}

// ─── GOALS ──────────────────────────────────────────────────────────
// Single file: Krysalis OS/Goals.md with task-list checkboxes
...
// ─── JOURNAL ────────────────────────────────────────────────────────
// One file per day: Krysalis OS/Journal/YYYY-MM-DD.md
