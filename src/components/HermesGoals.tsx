...
function statusBadge(s: GoalStatus) {
  const map: Record<GoalStatus, { bg: string; fg: string; icon: React.ReactNode; label: string }> = {
    ...
    completed: { bg: "rgba(90,184,150,0.18)", fg: "var(--accent-secondary)",       icon: <CheckCircle2 size={11} />, label: "done" },
    failed:    { bg: "rgba(196,96,126,0.18)", fg: "var(--accent-danger)",          icon: <AlertCircle size={11} />, label: "failed" },
    stopped:   { bg: "rgba(201,124,94,0.18)", fg: "var(--accent-warning)",          icon: <Pause size={11} />, label: "stopped" },
  };
  const c = map[s];
  return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-4">
        ...
        <div className="space-y-4">
          <div className="panel p-4 space-y-2">
            ...
            {error && <div className="mt-1 text-[11px] text-[var(--accent-danger)]">{error}</div>}
          </div>
                      ...
                      <div className="flex items-center gap-2">
                        {g.status === "running" && (
                          <button onClick={(e) => { e.stopPropagation(); stopOne(g.id); }}
                            className="hover:text-[var(--accent-danger)]" title="Stop">
                            <Square size={11} />
                          </button>
                        )}
                        <button onClick={(e) => { e.stopPropagation(); deleteOne(g.id); }}
                          className="hover:text-[var(--accent-danger)]" title="Delete">
                          <Trash2 size={11} />
                        </button>
                      </div>
                    </div>
