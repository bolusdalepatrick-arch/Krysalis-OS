...
//   Workspace : browse everything ever rendered (Krysalis OS + Hermes + Downloads)
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-4">
      <PromptCard title="Create video · HyperFrames" accent={ACCENT}>
        ...
        <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)}
          placeholder="A 10-second cinematic intro for Krysalis OS — fade in title, ambient particles, gold-on-aubergine palette"
          rows={5}
        ...
        {error && <div className="text-[11px] text-[var(--accent-danger)] mt-2" title={error}>{error.slice(0, 220)}</div>}
        ...
        <div className="mt-auto pt-3 border-t" style={{ borderColor: "var(--line-soft)" }}>
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={loadProjects} aria-label="Refresh Projects" className="text-[var(--cream-mute)] hover:text-[var(--cream)]"><RefreshCw size={11} /></button>
          </div>
      ...
      <PreviewCard title={openJob ? `Render · ${openJob.job.status}` : (active ? active.slug : "Preview")} accent={ACCENT}>
        {openJob ? (
          ...
          <EmptyState icon={<Film size={28} style={{ color: ACCENT }} />} title="Make a video"
            hint="Describe what you want on the left. Krysalis OS scaffolds an HTML composition + immediately runs `hyperframes render`. First render takes 30–90s depending on length." />
        )}
      </PreviewCard>
...
function RenderJobView({ job, log, outputUrl, onClose }: { job: RenderJob; log: string; outputUrl?: string; onClose: () => void }) {
  const isLoading = job.status === "rendering" || job.status === "queued";
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col gap-2">
      <div className="flex items-center justify-between text-[11px]" style={{ color: "var(--cream-mute)" }}>
        ...
        <button onClick={onClose} aria-label="Close" className="hover:text-[var(--cream)]"><XIcon size={12} /></button>
      </div>
      {outputUrl ? (
        ...
        <div className="flex-1 grid place-items-center text-center p-6">
          <div>
            {isLoading ? (
              ...
              <CheckCircle2 size={28} className="mx-auto mb-2" style={{ color: "var(--accent-secondary)" }} />
            ) : (
              <AlertCircle size={28} className="mx-auto mb-2" style={{ color: "var(--accent-danger)" }} />
            )}
            <div className="text-[13px] text-[var(--cream)] font-medium mb-1">
              {isLoading ? "Rendering MP4…" : job.status === "queued" ? "Queued…" : isComplete ? "Render complete" : `Status: ${job.status}`}
            </div>
            {job.lastOutput && (
        ...
        {jobs.slice(0, 10).map((j) => (
          <button key={j.id} onClick={() => onOpen(j)}
            ...
            <div className="flex items-center gap-3 text-[10px] mono shrink-0 ml-2" style={{ color: "var(--cream-mute)" }}>
              ...
              <span style={{ color: j.status === "completed" ? "var(--accent-secondary)" : j.status === "failed" ? "var(--accent-danger)" : ACCENT }}>
                {j.status}
              </span>
            </div>
          </button>
...
function AvatarTab() {
  ...
  const [avatarQuery, setAvatarQuery] = useState("Krysalis OS");  // default — your clones first
  const [voiceQuery, setVoiceQuery] = useState("Krysalis OS");
      ...
      <PromptCard title="Avatar · AI Studio" accent={ACCENT}>
        ...
        <div className="space-y-2 mt-3">
          ...
          <textarea value={text} onChange={(e) => setText(e.target.value)}
            placeholder="Hey everyone, welcome to my new Krysalis OS dashboard. This is the live AI-agent command centre I run my whole business through…"
            rows={3}
          ...
          {error && <div className="text-[11px] text-[var(--accent-danger)]">{error.slice(0, 220)}</div>}
        </div>
      </PromptCard>

      <PreviewCard title={job ? `Avatar video · ${job.status}` : "Preview"} accent={ACCENT}>
        {job ? (() => {
          const isLoading = job.status === "pending" || job.status === "waiting" || job.status === "processing";
          const isComplete = job.status === "completed";
            ...
            {job.videoUrl ? (
              ...
              <div className="flex-1 grid place-items-center text-center p-6">
                <div>
                  ...
                  <div className="text-[13px] text-[var(--cream)] font-medium mb-1">
                    {isLoading ? "Rendering avatar video…" : isComplete ? "Done" : `Status: ${job.status}`}
                  </div>
          ...
          <EmptyState icon={<User size={28} style={{ color: ACCENT }} />} title="Pick an avatar + voice → script → Generate"
            hint="AI avatar renders typically take 30–120 seconds. Your existing Krysalis OS SEO clones are pre-filtered. Status polls every 5s." />
        )}
      </PreviewCard>
          ...
          <button key={it.videoId} onClick={() => onLoad(it)}
            ...
            <div className="p-2 space-y-0.5">
              ...
              <div className="flex items-center justify-between text-[9.5px] mono" style={{ color: "var(--cream-mute)" }}>
                ...
                <span style={{ color: it.status === "completed" ? "var(--accent-secondary)" : it.status === "failed" ? "var(--accent-danger)" : ACCENT }}>{it.status}</span>
              </div>
            </div>
          </button>
        ))}
    ...
    <div className={`grid grid-cols-1 ${open ? "lg:grid-cols-[260px_280px_1fr]" : "lg:grid-cols-[320px_1fr]"} gap-0 h-full min-h-[600px] border rounded-md overflow-hidden`}
      ...
      <aside className="border-r p-3 space-y-2 overflow-y-auto scroll" style={{ borderColor: "var(--line-soft)" }}>
        <div className="flex items-center justify-between mb-1">
          ...
          <button onClick={loadBuckets} aria-label="Refresh Buckets" className="text-[var(--cream-mute)] hover:text-[var(--cream)]"><RefreshCw size={11} /></button>
        </div>
