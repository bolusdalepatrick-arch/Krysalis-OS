...
import { Activity, Box, Sparkles, Cpu, Zap, Route, Server, HardDrive } from "lucide-react";
...
interface VitalsData {
  ...
  hardware: { cpuUsage: number; freeMemGb: number; totalMemGb: number; uptimeSec: number };
}
...
function VitalTile({
  label, icon, primary, sub, status, href, loading
}: {
  label: string;
  icon: ReactNode;
  primary: ReactNode;
  ...
  const inner = (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {!loading && <span className={`status-dot ${status}`} />}
      </div>
      {loading ? (
        <div className="h-7 w-20 bg-[rgba(255,255,255,0.05)] animate-pulse rounded my-2" />
      ) : (
        <>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
      <VitalTile
        ...
        sub={data ? `v${data.claude.version}` : "checking…"}
      ...
      <VitalTile
        ...
        icon={<Box size={12} />}
        href="/openclaw"
        primary={!data?.openclaw.ok ? "Offline" : data.openclaw.degraded ? "Degraded" : "Online"}
      ...
      <VitalTile
        label="VPS CPU"
        icon={<Server size={12} />}
        primary={data ? <>{data.hardware.cpuUsage}<span className="text-[var(--cream-dim)] text-[0.7em] ml-0.5">%</span></> : "…"}
        sub={data ? `up ${Math.floor(data.hardware.uptimeSec / 3600)}h ${Math.floor((data.hardware.uptimeSec % 3600) / 60)}m` : "checking…"}
        status={data ? (data.hardware.cpuUsage > 85 ? "warn" : "ok") : "info"}
        loading={!data}
      />
      <VitalTile
        label="VPS RAM"
        icon={<HardDrive size={12} />}
        primary={data ? <>{data.hardware.totalMemGb.toFixed(1)}<span className="text-[var(--cream-dim)] text-[0.7em] ml-0.5">GB</span></> : "…"}
        sub={data ? `${data.hardware.freeMemGb.toFixed(1)}GB free` : "checking…"}
        status={data ? (data.hardware.freeMemGb < 1 ? "warn" : "ok") : "info"}
        loading={!data}
      />
    </div>
  );
