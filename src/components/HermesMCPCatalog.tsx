    ...
    <div className="space-y-4">
      ...
      {error && (
        <div className="flex items-start gap-2 text-[12px] p-3 rounded-md border"
             style={{ borderColor: "var(--accent-danger)", background: "rgba(196,96,126,0.08)", color: "var(--cream)" }}>
                        ...
                        {incompatible ? (
                          ...
                          <>
                            <button onClick={() => copyInstall(c.name)}
                                    ...
                                    style={{ color: copied === c.name ? "var(--accent-secondary)" : "var(--cream-mute)" }}>
                  ...
                  <div className="flex items-center justify-between gap-2 mb-1.5 flex-wrap">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => toggle(i.name, !i.enabled)} disabled={busy === i.name}
                              ...
                              style={{ color: i.enabled ? "var(--accent-secondary)" : "var(--cream-mute)" }}>
                      ...
                      <button onClick={() => doUninstall(i.name)} disabled={busy === i.name}
                              ...
                              className="text-[10px] uppercase tracking-widest px-2 py-1 rounded hover:text-[var(--accent-danger)] disabled:opacity-50">
                        <Trash2 size={11} />
                      </button>
                    </div>
                  </div>
            ...
            <>
              <div ref={logBoxRef}
                ...
                {log.map((ev, i) => {
                  ...
                  if (ev.type === "error") return <div key={i} style={{ color: "var(--accent-danger)" }}>✗ {ev.text}</div>;
                  if (ev.type === "done") return (
                    <div key={i} style={{ color: ev.ok ? "var(--accent-secondary)" : "var(--accent-danger)" }}>
              ...
              {phase === "done" && (
                <div className="flex items-center justify-end gap-2 pt-2">
                  ...
                  <button onClick={() => onComplete()}
                          ...
                          style={{
                            color: installOk ? "var(--accent-secondary)" : "var(--cream)",
                            border: `1px solid ${installOk ? "var(--accent-secondary)" : "var(--cream-mute)"}55`,
...
function StatusPill({ status, alreadyInstalled }: { status: string; alreadyInstalled: boolean }) {
  ...
  const color = isEnabled ? "var(--accent-secondary)" : isInstalled ? ACCENT : "var(--cream-mute)";
      ...
      <div className="panel w-full max-w-[720px] max-h-[90vh] flex flex-col overflow-hidden"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto scroll p-4 space-y-4">
          ...
          <div>
            ...
            {presetMeta?.argHint && (
              <div className="text-[10.5px] mt-1.5 text-[var(--accent-primary)] flex items-start gap-1">
          ...
          <div>
            ...
            <input value={name}
                   ...
                   className="w-full px-3 py-2 text-[12px] rounded-md border bg-transparent text-[var(--cream)] mono"
                   style={{ borderColor: nameClash ? "var(--accent-danger)" : "var(--line-soft)" }} />
            {name && !nameValid && (
              <div className="text-[10.5px] mt-1 text-[var(--accent-danger)]">Name must match {"[a-zA-Z0-9_-]{1,64}"}</div>
            )}
            {nameClash && (
              <div className="text-[10.5px] mt-1 text-[var(--accent-danger)]">A server with this name is already installed.</div>
            )}
          </div>
              ...
              {envVarRows.map((row, idx) => (
                <div key={idx} className="flex items-start gap-2">
                  ...
                  <button onClick={() => removeEnvRow(idx)}
                          className="text-[var(--cream-mute)] hover:text-[var(--accent-danger)] mt-1.5 p-1"
          ...
          {result && (
            <div className="rounded-md border p-3 text-[11.5px] space-y-1"
                 style={{
                   borderColor: result.ok ? "var(--accent-secondary)" : "var(--accent-danger)",
                   background: result.ok ? "rgba(90,184,150,0.08)" : "rgba(196,96,126,0.08)",
                   color: "var(--cream)",
                 }}>
              <div className="flex items-center gap-2 font-medium">
                {result.ok ? <><CheckCircle2 size={13} style={{ color: "var(--accent-secondary)" }} /> Added successfully</> : <><AlertCircle size={13} style={{ color: "var(--accent-danger)" }} /> Add failed</>}
              </div>
                ...
                {tools.map((t) => (
                  <span key={t}
                    ...
                    <button onClick={() => removeTool(t)}
                            className="hover:text-[var(--accent-danger)]" title={`Remove ${t}`}>
                      <XIcon size={11} />
                    </button>
                  </span>
                ))}
