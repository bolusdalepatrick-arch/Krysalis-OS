...
type SR = { lang: string; interimResults: boolean; continuous: boolean; onresult: ((e: { results: ArrayLike<ArrayLike<{ transcript: string }> & { isFinal: boolean }> }) => void) | null; onend: (() => void) | null; onerror: (() => void) | null; start: () => void; stop: () => void };
...
export default function HermesTalk() {
  ...
  const scheduleListen = (ms: number) => {
    ...
    timerRef.current = setTimeout(() => {
      if (convoRef.current) beginListen();
    }, ms);
  };
  ...
  useEffect(() => {
    ...
    r.onerror = () => { setListening(false); if (convoRef.current) scheduleListen(500); };
    recRef.current = r;
    return () => { 
      try { r.stop(); } catch { /* */ } 
      if (timerRef.current) clearTimeout(timerRef.current);
  ...
  function beginListen() {
    ...
    r.onend = () => {
      ...
      setInterim("");
      if (t) send(t);
      else if (convoRef.current) scheduleListen(400); // user paused — keep waiting
    };
    try { r.start(); } catch { scheduleListen(600); }
  }
  ...
  async function send(text: string) {
    ...
    try {
      ...
      if (j.audio) {
        ...
        a.onended = () => { setSpeaking(false); if (convoRef.current) scheduleListen(250); };
        a.play().catch(() => { setSpeaking(false); if (convoRef.current) scheduleListen(250); });
      ...
      setBusy(false);
      if (convoRef.current) scheduleListen(400);
    }
  }
  ...
  const status = !convo ? (sttOk ? "Tap to start a hands-free conversation" : "Voice needs Chrome — type below")
