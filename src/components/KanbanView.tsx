...
export default function KanbanView() {
  ...
  const [newAssignee, setNewAssignee] = useState("krysalisos"); // default to user's active profile
              ...
              <div className="space-y-2 flex-1 min-h-0">
                  {!board ? (
                    <div className="space-y-2">
                      <div className="w-full h-[72px] rounded-lg bg-[rgba(255,255,255,0.02)] animate-pulse border border-[var(--panel-border)]" />
                      <div className="w-full h-[72px] rounded-lg bg-[rgba(255,255,255,0.02)] animate-pulse border border-[var(--panel-border)]" />
                    </div>
                  ) : tasks.length === 0 ? (
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-5 py-4 border-b border-[var(--panel-border)] bg-[rgba(0,0,0,0.25)]">
        <div className="flex items-start justify-between gap-3">
          ...
          <button onClick={onClose} aria-label="Close" className="text-[var(--fg-dim)] hover:text-[var(--fg)] shrink-0"><X size={18}/></button>
        </div>
        ...
        <div className="mt-3 flex items-center gap-3 flex-wrap text-[11.5px]">
          ...
          {t.workspace_path && (
            <span className="font-[var(--font-geist-mono)] text-[10px] text-[var(--fg-dimmer)] truncate" title={t.workspace_path}>
              ws: {t.workspace_path.replace(process.env.HOME ?? "/Users/krysalisos", "~")}
            </span>
          )}
        </div>
      </div>
        ...
        <section>
          ...
          {wsRoot && (
            <div className="flex items-center gap-1.5 mb-2 text-[10.5px] font-[var(--font-geist-mono)] text-[var(--fg-dimmer)]">
              <span className="truncate" title={wsRoot}>{wsRoot.replace("/Users/krysalisos", "~")}</span>
              <button
