...
// Embeds the real Hermes web dashboard (FastAPI on :9119) inside Krysalis OS so you
// can manage everything — model/provider, API keys, sessions, cron, skills, MCP,
// channels, logs, analytics, system ops — without leaving the house UI.
      ...
      <div className="flex-1 min-h-[640px] rounded-2xl border border-[var(--panel-border)] overflow-hidden relative bg-[#0a0518]">
        {state === "up" ? (
          <iframe
            ref={iframeRef}
            key={bust}
            src={`${DASH_URL}/?embed=krysalisos`}
            title="Hermes Dashboard"
            className="absolute inset-0 w-full h-full border-0"
            // Allow the embedded SPA full function (its own same-origin fetches, clipboard for tokens).
          ...
          <div className="absolute inset-0 grid place-items-center p-8 text-center">
            <div className="max-w-md space-y-3">
              ...
              <div className="text-[11px] text-[var(--fg-dimmer)]">Or run <code>hermes dashboard --tui</code> in a terminal.</div>
            </div>
          </div>
        ) : (
