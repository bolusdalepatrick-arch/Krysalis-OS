...
export default function MemoryPanel() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`/api/memory/search?q=${encodeURIComponent(debouncedQ)}`, {
    ...
    <Panel
      ...
      actions={
        <span className="pill pill-info">
          {vaultStats ? `${vaultStats.omi} omi · ${vaultStats.notes} notes` : "Loading..."}
        </span>
      }
      ...
      {tab === "graph" ? (
        <div className="flex flex-col h-full min-h-0">
          ...
          <div className="flex-1 min-h-[600px] rounded-2xl border border-[var(--panel-border)] overflow-hidden relative bg-black">
            <ErrorBoundary>
              <VaultGraph3D onOpenNote={openNote} />
            </ErrorBoundary>
          </div>
          {open && (
        ...
        <div className="lg:w-[380px] flex flex-col min-h-0 shrink-0">
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-[var(--panel-border)] bg-[rgba(0,0,0,0.25)] mb-3">
            ...
            <input
              ...
              placeholder={vaultStats ? `Search ${vaultStats.omi} memories + ${vaultStats.notes} notes…` : "Search memories and notes…"}
